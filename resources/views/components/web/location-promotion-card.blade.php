...
<div class="min-[768px]:w-[25%] w-[33.33%] max-[480px]:w-full px-[12px] mb-[24px] pro-bb-content" data-aos="fade-up" data-aos-duration="1000" data-aos-delay="200">
        ...
        <div class="bb-pro-img overflow-hidden relative border-b-[1px] border-solid border-[#eee] z-[4]">
            ...
            <a href="{{ route('promociones.mostrar',['catalogo'=>$promotion->catalog->slug,'slug'=>$promotion->slug])}}">
                ...
                <div class="inner-img relative block overflow-hidden pointer-events-none rounded-t-[20px]">                   
                    ...
                    <div class="bb-offers py-[5px] px-[15px] absolute top-[5px] right-[5px] bg-[#ff2525] opacity-[0.8] rounded-[15px]">
                        <span class="text-[10px] font-bold text-[#fff]">- {{ $promotion->discount }}%</span>
                    </div>
            ...
            <!-- Imagen regalo -->
            <div class="vendors-local-shape absolute bottom-[0] right-[0] h-[40%] w-[40%] bg-[#fff] pt-[20x] pl-[10px] rounded-tl-[20px]">                          
                <img loading="lazy" src="{{asset($promotion->catalog->complement)}}" alt="vendor" class="w-[100%] h-[100%] rounded-[20px] border-[1px] border-solid border-[#eee]">
            </div>
            <!-- boton comprar -->
        ...
        <div class="bb-pro-contact p-[20px]">
            <div class="bb-pro-subtitle mb-[8px] flex flex-wrap justify-start">  
                    ...
                    @foreach ($promotion->details->first()->product->tags as $tag)
                        <span href="#pendiente" aria-hidden="true" class="text-[9px]">  {{ $tag->tag->tag }}</span>
                        <span href="#pendiente" aria-hidden="true" class="text-[9px] ml-1 mr-1"> | </span>
                    @endforeach               
            </div>

            <h4 class="bb-pro-title mb-[8px] text-[16px] leading-[18px]">
            <a href="{{ route('promociones.mostrar',['catalogo'=>$promotion->catalog->slug,'slug'=>$promotion->slug]) }}" class="transition-all duration-[0.3s] ease-in-out font-quicksand w-full block whitespace-nowrap overflow-hidden text-ellipsis text-[15px] leading-[18px] text-[#162c48] font-bold tracking-[0.03rem]">{{ $promotion->name }}</a></h4>

            <div class="bb-price flex flex-wrap justify-between">
                <div class="inner-price mx-[-3px]">                    
                    <span class="px-[3px] text-ellipsis line-clamp-2  text-xs text-[#686e7d] font-light"> {{ $promotion->description }}</span>                    
                </div>
            </div>
        </div>
